...
par(mfrow=c(2,2))
plot(Sockeyelong_all$Gage_Height, Sockeyelong_all$Q_cfs, main = "Sockeye Q_cfs")
plot(Sockeyelong_all$Gage_Height, Sockeyelong_all$Q_cfs2, main = "Sockeye Q_cfs2")

# Coholong_all <- Coholong_all %>% mutate(Q_cfs2 = 7.48*(Gage_Height - 20.33)^2.5) # coefficients are from the excel spreadsheet:  Salmon_2015_qPCR_count_Qcfs_20180504.xlsx
Coholong_all <- Coholong_all %>% mutate(Q_cfs2 = a*(Gage_Height - b)^c)
plot(Coholong_all$Gage_Height, Coholong_all$Q_cfs, main = "Coho Q_cfs")
plot(Coholong_all$Gage_Height, Coholong_all$Q_cfs2, main = "Coho Q_cfs2")
par(mfrow=c(1,1))           
names(Sockeyelong_all)
names(Coholong_all)
...
timelines_full <- function(salmonspecies, start = "2015-01-01", end = "2015-12-31", type = "_adult") { # x == Sockeye, Coho, y == _adult, _juv
    ...
    qpcrcorrQ <- ggplot(salmon_use, aes(Date, Qcorr_qPCR)) + geom_point(size = 0.5) + geom_line() + xlab("") + ylab("Qcfs2*QUANT") + background_grid(major = "x", minor="x", size.minor = 0.5, colour.minor = "grey90")
    ...
    # plot_grid(counts, qpcrcorrQ, qpcr, Q_cfs2, temp, nrow = 5, align = "v")
    plot_grid(counts, qpcrcorrQ, QUANT, Q_cfs2, temp, nrow = 5, align = "v")
}
```

```{r 4 timelines_full usage 2015}
timelines_full("sockeye") # use sockeye, not Sockeye.  This defaults to the 2015 adult datasets
timelines_full("sockeye", "2015-06-18", "2015-08-01") # after looking at the full timeline, truncating to 01 Aug to prevent measuring dead-salmon DNA in 2016.  Adults only
...
```{r 4 timelines_full usage 2016}
timelines_full("sockeye", "2016-06-25", "2016-08-01") # after looking at the full timeline, truncating to 01 Aug to prevent measuring dead-salmon DNA in 2016.  Adults only
...
# timelines_full("coho", "2016-04-15", "2016-06-10", "_juv") # no qpcr data.  downstream weir dates only
```

```{r 4 timelines_full usage 2014}
timelines_full("sockeye", "2014-06-01", "2014-08-31") # after looking at the full timeline, truncating to 01 Aug to prevent measuring dead-salmon DNA in 2016.  Adults only
# timelines_full("sockeye", "2014-04-15", "2014-06-10", "_juv") # no qpcr data.  downstream weir dates only for smolts
timelines_full("coho", "2014-06-01", "2014-10-30") # after looking at the full timeline
# timelines_full("coho", "2014-04-15", "2014-06-10", "_juv") # no qpcr data.  downstream weir dates only
```
...
plot(simulationOutput, quantreg = T) # are uniform except for lowest predicted values
testUniformity(simulationOutput) # p = 0.3612, D = 0.221, overdispersion ns
...
testTemporalAutocorrelation(simulationOutput = simulationOutput, time = datasubset$Date, plot = T) # p = 0.472, DW = 1.65, temp autocorr ns (although the plot shows that residuals increase over time)
testTemporalAutocorrelation(simulationOutput = simulationOutput,  plot = T) # p = 0.8432 for a random order of datapoints (null expectation of p for truly non-temp-corr data)
...
plot(simulationOutput, quantreg = T) # overdispersed and non-normal residuals
testUniformity(simulationOutput) # p = 0.00015, D = 0.348, overdispersaion highly sig

# plot residuals against predictors
par(mfrow = c(2,2))
plotResiduals(datasubset$Temp_C,  simulationOutput$scaledResiduals) # a bit of overdispersion
plotResiduals(datasubset$QUANT_mean,  simulationOutput$scaledResiduals) # overdispersion
...
testTemporalAutocorrelation(simulationOutput = simulationOutput, time = datasubset$Date, plot = T) # p = 0.342, DW = 1.70, temp autocorr ns (although the plot shows that residuals increase over time)
testTemporalAutocorrelation(simulationOutput = simulationOutput,  plot = T) # p = 0.216 for a random order of datapoints (null expectation of p for truly non-temp-corr data)

tidy(sockeye.adult.2015.reg)
tidy(sockeye.adult.2016.reg)

# scatterplot of fitted ~ observed
plot(augment(sockeye.adult.2015.reg)$.fitted ~ augment(sockeye.adult.2015.reg)$Count)
plot(augment(sockeye.adult.2016.reg)$.fitted ~ augment(sockeye.adult.2016.reg)$Count)
...
plot(simulationOutput, quantreg = T) # overdispersed and non-normal residuals
testUniformity(simulationOutput) # p = 0.0006121, D = 0.267, overdispersaion highly sig
...
plotResiduals(datasubset$QUANTQ,  simulationOutput$scaledResiduals) # overdispersion
par(mfrow = c(1,1))

testTemporalAutocorrelation(simulationOutput = simulationOutput, time = datasubset$Date, plot = T) # p = 0.044, DW = 1.47, temp autocorr 
testTemporalAutocorrelation(simulationOutput = simulationOutput, plot = T) # p = 0.670 for a random order of datapoints (null expectation of p for truly non-temp-corr data)
...
# 1       (Intercept) 200.3557 211.23011  0.9485186 3.491898e-01
# 2            Temp_C -11.1493  12.32265 -0.9047809 3.715969e-01
# 3 QUANT_mean:Q_cfs2 527.0809  73.22268  7.1983282 1.806466e-08
...
tidy(sockeye.adult.2015.reg)
#                term  estimate std.error statistic      p.value
# 1       (Intercept) 634.67546 264.19205  2.402326 0.0319456027
# 2            Temp_C -35.39864  15.62974 -2.264826 0.0412614833
# 3 QUANT_mean:Q_cfs2 696.66077 155.88840  4.468971 0.0006322252
...
plot(datasubset$Date[which(datasubset$Year==2015)], datasubset$Count[which(datasubset$Year==2015)], type='l', ylim=c(0,600), col="blue")
...
mtext("Sockeye Counts Gaussian models",side=2,cex=1,outer=T)
par(mfrow=c(1,1))
...
augment(sockeye.adult.2015.reg)
tidy(sockeye.adult.2016.reg)

# scatterplot of fitted ~ observed
plot(exp(augment(sockeye.adult.2015.reg)$.fitted) ~ augment(sockeye.adult.2015.reg)$Count)
plot(exp(augment(sockeye.adult.2016.reg)$.fitted) ~ augment(sockeye.adult.2016.reg)$Count)
...
tidy(sockeye.adult.2016.reg)
#                term   estimate std.error statistic      p.value
# 1       (Intercept)  7.0706885 3.7644405  1.878284 6.846031e-02
# 2            Temp_C -0.2377161 0.2234705 -1.063747 2.945266e-01
# 3 QUANT_mean:Q_cfs2  3.9933579 0.8182676  4.880259 2.165027e-05
coef(sockeye.adult.2016.reg)[1] # intercept
coef(sockeye.adult.2016.reg)[2] # Temp_C
coef(sockeye.adult.2016.reg)[3] # QUANT_mean:Q_cfs
...
tidy(sockeye.adult.2015.reg)
#                term   estimate std.error statistic     p.value
# 1       (Intercept) 10.9280579 2.7423064  3.984988 0.001555447
# 2            Temp_C -0.4122434 0.1753893 -2.350448 0.035196689
# 3 QUANT_mean:Q_cfs2  3.1906301 0.9705787  3.287348 0.005889354
coef(sockeye.adult.2015.reg)[1] # intercept
coef(sockeye.adult.2015.reg)[2] # Temp_C
coef(sockeye.adult.2015.reg)[3] # QUANT_mean:Q_cfs
...
lines(datasubset$Date[which(datasubset$Year==2015)], 
    ...
    coef(sockeye.adult.2015.reg)[3] * datasubset$QUANT_mean[which(datasubset$Year==2015)] * datasubset$Q_cfs2[which(datasubset$Year==2015)]), lty=2, col="blue")
text(datasubset$Date[which(datasubset$Year==2015)][14.5],550,"2015",cex=1.5) 
mtext("Sockeye Counts Quasipoisson models",side=2,cex=1,outer=T)
par(mfrow=c(1,1))
...
# estimate residuals with DHARMa
simulationOutput <- simulateResiduals(fittedModel = sockeye.adult.2015.reg, n = 1000)
plot(simulationOutput, quantreg = T) # residuals look underdispersed. This leads to overwide CIs and p-values too small (so we should be more wary of concluding non-significance of a variable)
testUniformity(simulationOutput) # p = 0.886, D = 0.14, underoverdispersaion ns
...
testTemporalAutocorrelation(simulationOutput = simulationOutput, time = datasubset$Date, plot = T) # p = 0.551, DW = 1.71, temp autocorr ns (although the plot shows that residuals increase over time)
testTemporalAutocorrelation(simulationOutput = simulationOutput,  plot = T) # p = 0.952 for a random order of datapoints (null expectation of p for truly non-temp-corr data)
...
# summary(sockeye.adult.2015.reg)
# simulationOutput <- simulateResiduals(fittedModel = sockeye.adult.2015.reg, n = 1000)
# plot(simulationOutput) # residuals worse than model without Temp_C
...
summary(sockeye.adult.2016.reg)
simulationOutput <- simulateResiduals(fittedModel = sockeye.adult.2016.reg, n = 1000)
plot(simulationOutput) # very few data at high predicted values, but at low predicted values, residuals are nicely uniformly distributed. # we could say that residuals are a bit underdispersed. This leads to overwide CIs and p-values too small (so we should be more wary of concluding non-significance of a variable)
# testUniformity(simulationOutput) # p = 0.021, D = 0.24, underoverdispersaion ns
testDispersion(simulationOutput)  # p = 0.75, dispersion = 0.872
testZeroInflation(simulationOutput)  # p = 0.002, ratioObsExp = 0
...
testTemporalAutocorrelation(simulationOutput = simulationOutput, time = datasubset$Date, plot = T) # p = 0.1113, DW = 1.50, temp autocorr not sig:  more positive residuals later, so should I truncate the time series a bit?  and if so, where?
...
# DHARMa residuals
simulationOutput <- simulateResiduals(fittedModel = sockeye.adult.reg, n = 1000)
plot(simulationOutput) # very few data at high predicted values, but at the low predicted values, residuals are nicely uniformly distributed. # we could say that residuals are a bit underdispersed. This leads to overwide CIs and p-values too small (so we should be more wary of concluding non-significance of a variable)
# testUniformity(simulationOutput) # p = 0.083, D = 0.170, underoverdispersaion ns
testDispersion(simulationOutput)  # p = 0.694, dispersion = 0.935, too conservative a test but the parametric version doesn't work for this kind of model
testZeroInflation(simulationOutput)  # p < 2.2e-16, ratioObsExp = 0
...
testTemporalAutocorrelation(simulationOutput = simulationOutput, time = datasubset$Date, plot = T) # p = 0.044, DW = 1.47, temp autocorr marginally sig:  more positive residuals later, so should I truncate the time series a bit?  and if so, where?
testTemporalAutocorrelation(simulationOutput = simulationOutput,  plot = T) # p = 0.795, DW = 2.13 for a random order of datapoints (null expectation of p for truly non-temp-corr data)
...
# estimate residuals with DHARMa
simulationOutput <- simulateResiduals(fittedModel = coho.adult.2015.reg, n = 1000)
plot(simulationOutput, quantreg = T) # most predicted points are small and they are uniformly distributed
...
# DHARMa residuals
simulationOutput <- simulateResiduals(fittedModel = coho.adult.2016.reg, n = 1000)
plot(simulationOutput) # a bit heteroscedastic
...
simulationOutput <- simulateResiduals(fittedModel = coho.adult.reg, n = 1000)
plot(simulationOutput, quantreg = T) # a bit overdispersed
...
plot(simulationOutput) # too many residuals around 0 and 1
...
summary(fittedModel)
simulationOutput <- simulateResiduals(fittedModel = fittedModel)
plot(simulationOutput) # too many residuals around 0.5
testUniformity(simulationOutput = simulationOutput)
