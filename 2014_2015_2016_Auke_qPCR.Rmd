...
sockeye_adult <- sockeye_adult %>% mutate(Q_cfs2 = a*(Gage_Height - b)^c) 
par(mfrow=c(2,2))
plot(sockeye_adult$Gage_Height, sockeye_adult$Q_cfs, main = "Sockeye Q_cfs")
plot(sockeye_adult$Gage_Height, sockeye_adult$Q_cfs2, main = "Sockeye Q_cfs2")
...
coho_adult <- coho_adult %>% mutate(Q_cfs2 = a*(Gage_Height - b)^c)
plot(coho_adult$Gage_Height, coho_adult$Q_cfs, main = "Coho Q_cfs")
plot(coho_adult$Gage_Height, coho_adult$Q_cfs2, main = "Coho Q_cfs2")
par(mfrow=c(1,1))                                                 
...
plot(simulationOutput = simulationOutput, quantreg = T) # are uniform except for lowest predicted values
testUniformity(simulationOutput) # p = 0.3612, D = 0.221, overdispersaion ns
...
testTemporalAutocorrelation(simulationOutput = simulationOutput,  plot = T) # p = 0.5711 for a random order of datapoints (null expectation of p for truly non-temp-corr data)
...
testTemporalAutocorrelation(simulationOutput = simulationOutput, time = datasubset$Date, plot = T) # p = 0.034, DW = 1.70, temp autocorr ns (although the plot shows that residuals increase over time)
testTemporalAutocorrelation(simulationOutput = simulationOutput,  plot = T) # p = 0.678 for a random order of datapoints (null expectation of p for truly non-temp-corr data)
...
testTemporalAutocorrelation(simulationOutput = simulationOutput, plot = T) # p = 0.192 for a random order of datapoints (null expectation of p for truly non-temp-corr data)
